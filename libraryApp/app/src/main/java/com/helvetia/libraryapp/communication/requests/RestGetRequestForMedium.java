 ...
 * @version 1.0.0
 */
public class RestGetRequestForMedium extends AsyncTask<String, Integer, String> {
    private final AsyncTableListener<Medium> listener;
    ...
    public RestGetRequestForMedium(AsyncTableListener<Medium> listener) {
    ...
    protected String doInBackground(String... params) {
        ...
        try {
            URL url = new URL(SingletonAppData.getInstance().getApiURL() + "/medium");
    ...
    protected void onPostExecute(String result) {
        if (responseCode == HttpURLConnection.HTTP_OK) {
            ...
            if (result.endsWith("]")) {
                medien.addAll(Medium.parseJsonObjects(result));
            } else {
                medien.add(Medium.parseJsonObject(result, Medium.class));
            }
